...
const TabBarCustomButton = ({accessibilityLabel, accessibilityState, children, onPress}) => {
    if(accessibilityState.selected){
        ...
        return (
            <TouchableOpacity style= {{flex: 1, justifyContent: 'center', alignItems: 'center', height: 50, width: 50, backgroundColor: COLORS.white}}
            onPress={onPress}
            activeOpacity= {1}
        ...
        >
          ...
          options= {{
            tabBarIcon: ({focused}) => (
                ...
                source= {icons.more}
                resizeMode= "contain"
                style= {{width:25, height: 25, tintColor: focused ? COLORS.green : COLORS.secondary}}
                />
            ),
            tabBarButton: (props) => (
